...
EbErrorType generate_picture_window_split(
    ...
    while (MiniGopIndex < MINI_GOP_MAX_COUNT) {
        ...
        if (get_mini_gop_stats(MiniGopIndex)->end_index < encode_context_ptr->pre_assignment_buffer_count && context_ptr->mini_gop_activity_array[MiniGopIndex] == EB_FALSE) {
            ...
            printf("\n\nFound valid mini_gop, start/end/length %d/%d/%d, total_number_of_mini_gops %d, mini_gop_index %d\n",
        ...
        //} else {
        //    printf("didn't find valid mini_gop, why?, miniGopIndex %d, end_idx %d, buf count %d\n",
        //            MiniGopIndex,
...
EbErrorType handle_incomplete_picture_window_map(
    ...
    if (context_ptr->total_number_of_mini_gops == 0) {
        ...
        printf("Got incomplete window, start 0, end %d, total number of mini_gop %d\n",
                context_ptr->mini_gop_end_index[context_ptr->total_number_of_mini_gops],
                context_ptr->total_number_of_mini_gops + 1);
        context_ptr->total_number_of_mini_gops++;
    }
        ...
        printf("Got incomplete window, start %d, end %d, total number of mini_gop %d\n", 
                context_ptr->mini_gop_start_index[context_ptr->total_number_of_mini_gops],
                context_ptr->mini_gop_end_index[context_ptr->total_number_of_mini_gops],
                context_ptr->total_number_of_mini_gops + 1);
        context_ptr->total_number_of_mini_gops++;
...
EbErrorType GenerateMiniGopRps(
    ...
    for (mini_gop_index = 0; mini_gop_index < context_ptr->total_number_of_mini_gops; ++mini_gop_index) {
        ...
        for (pictureIndex = context_ptr->mini_gop_start_index[mini_gop_index]; pictureIndex <= context_ptr->mini_gop_end_index[mini_gop_index]; pictureIndex++) {
            ...
            //printf("In GenerateMiniGopRps, pred_struct_period is %d, hlevel %d\n",
...
static validate_rps(PredictionStructureEntry *predPositionPtr, Av1RpsNode *av1Rps)
{
    ...
    if (predPositionPtr->ref_list1.reference_list_count < 3) {
...
)
{
    ...
    else if (picture_control_set_ptr->hierarchical_levels == 1)
    {
        ...
        case 0:
            ...
            av1Rps->ref_dpb_index[LAST2] = base0_idx;
            av1Rps->ref_dpb_index[LAST3] = av1Rps->ref_dpb_index[LAST];
            av1Rps->ref_dpb_index[GOLD] = av1Rps->ref_dpb_index[LAST];

            av1Rps->ref_dpb_index[BWD] = av1Rps->ref_dpb_index[LAST];
            av1Rps->ref_dpb_index[ALT] = av1Rps->ref_dpb_index[LAST2];
            av1Rps->ref_dpb_index[ALT2] = av1Rps->ref_dpb_index[LAST];
            gop_i = 0;
            av1Rps->ref_poc_array[LAST] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, two_level_hierarchical_pred_struct[gop_i].ref_list0[0]);
            av1Rps->ref_poc_array[LAST2] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, two_level_hierarchical_pred_struct[gop_i].ref_list0[1]);
            av1Rps->ref_poc_array[LAST3] = av1Rps->ref_poc_array[LAST];
            av1Rps->ref_poc_array[GOLD] = av1Rps->ref_poc_array[LAST];

            av1Rps->ref_poc_array[BWD] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, two_level_hierarchical_pred_struct[gop_i].ref_list1[0]);
        ...
        case 1:
            //{ 1, 3, 5, 0}   // GOP Index 2 - Ref List 0
            //{-1, 1, 3, 0}   // GOP Index 2 - Ref List 1
            av1Rps->ref_dpb_index[LAST] = base1_idx;
            av1Rps->ref_dpb_index[LAST2] = base0_idx;
            av1Rps->ref_dpb_index[LAST3] = base2_idx;
            av1Rps->ref_dpb_index[GOLD] = av1Rps->ref_dpb_index[LAST];

            av1Rps->ref_dpb_index[BWD] = base2_idx;
            av1Rps->ref_dpb_index[ALT] = base1_idx;
            av1Rps->ref_dpb_index[ALT2] = base0_idx;

            gop_i = 1;
            av1Rps->ref_poc_array[LAST] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, two_level_hierarchical_pred_struct[gop_i].ref_list0[0]);
            av1Rps->ref_poc_array[LAST2] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, two_level_hierarchical_pred_struct[gop_i].ref_list0[1]);
            av1Rps->ref_poc_array[LAST3] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, two_level_hierarchical_pred_struct[gop_i].ref_list0[2]);
            av1Rps->ref_poc_array[GOLD] = av1Rps->ref_poc_array[LAST];

            av1Rps->ref_poc_array[BWD] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, two_level_hierarchical_pred_struct[gop_i].ref_list1[0]);
            av1Rps->ref_poc_array[ALT] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, two_level_hierarchical_pred_struct[gop_i].ref_list1[1]);
            av1Rps->ref_poc_array[ALT2] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, two_level_hierarchical_pred_struct[gop_i].ref_list1[2]);
        ...
        if ((pictureIndex == context_ptr->mini_gop_end_index[mini_gop_index] && !picture_control_set_ptr->is_overlay && sequence_control_set_ptr->static_config.pred_structure == EB_PRED_RANDOM_ACCESS) ||
    ...
    else if (picture_control_set_ptr->hierarchical_levels == 2)
    {
        ...
        case 0:
            ...
            av1Rps->ref_dpb_index[LAST] = base1_idx;
            av1Rps->ref_dpb_index[LAST2] = base0_idx;
            av1Rps->ref_dpb_index[LAST3] = base1_idx;//base2_idx;
            av1Rps->ref_dpb_index[GOLD] = base1_idx;

            av1Rps->ref_dpb_index[BWD] = av1Rps->ref_dpb_index[LAST];
            av1Rps->ref_dpb_index[ALT] = av1Rps->ref_dpb_index[LAST2];
            av1Rps->ref_dpb_index[ALT2] = av1Rps->ref_dpb_index[LAST3];
            gop_i = 0;
            av1Rps->ref_poc_array[LAST] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, three_level_hierarchical_pred_struct[gop_i].ref_list0[0]);
            av1Rps->ref_poc_array[LAST2] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, three_level_hierarchical_pred_struct[gop_i].ref_list0[1]);
            av1Rps->ref_poc_array[LAST3] = av1Rps->ref_poc_array[LAST];//get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, three_level_hierarchical_pred_struct[gop_i].ref_list0[2]);
            av1Rps->ref_poc_array[GOLD] = av1Rps->ref_poc_array[LAST];

            av1Rps->ref_poc_array[BWD] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, three_level_hierarchical_pred_struct[gop_i].ref_list1[0]);
            av1Rps->ref_poc_array[ALT] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, three_level_hierarchical_pred_struct[gop_i].ref_list1[1]);
            av1Rps->ref_poc_array[ALT2] = av1Rps->ref_poc_array[BWD];//get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, three_level_hierarchical_pred_struct[gop_i].ref_list1[2]);
        ...
        case 1:
            ...
            av1Rps->ref_poc_array[LAST] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, three_level_hierarchical_pred_struct[gop_i].ref_list0[0]);
            av1Rps->ref_poc_array[LAST2] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, three_level_hierarchical_pred_struct[gop_i].ref_list0[1]);
            av1Rps->ref_poc_array[LAST3] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, three_level_hierarchical_pred_struct[gop_i].ref_list0[2]);
            av1Rps->ref_poc_array[GOLD] = av1Rps->ref_poc_array[LAST];
        ...
        case 2:
            ...
            if (picture_control_set_ptr->is_overlay) {
                ...
                if (pictureIndex == 0) {
                    ...
                    av1Rps->ref_dpb_index[LAST2] = base1_idx;
                    av1Rps->ref_dpb_index[LAST3] = lay2_idx;
                    ...
                    av1Rps->ref_poc_array[LAST2] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, three_level_hierarchical_pred_struct[gop_i].ref_list0[1]);
                    av1Rps->ref_poc_array[LAST3] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, three_level_hierarchical_pred_struct[gop_i].ref_list0[2]);
                    av1Rps->ref_poc_array[GOLD] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, three_level_hierarchical_pred_struct[gop_i].ref_list0[3]);
            ...
            if (pictureIndex == 0 )
                av1Rps->refresh_frame_mask = 1 << (lay2_idx);
            else
                av1Rps->refresh_frame_mask = 0;
            break;
    ...
    else if (picture_control_set_ptr->hierarchical_levels == 3)
    {
        ...
        if (picture_control_set_ptr->pred_struct_ptr->pred_type == EB_PRED_LOW_DELAY_P || picture_control_set_ptr->is_overlay)
        {
            printf("Enter low delay P, POC %d\n", picture_control_set_ptr->picture_number);
            //P frames
            av1Rps->ref_dpb_index[4] = av1Rps->ref_dpb_index[5] = av1Rps->ref_dpb_index[6] = av1Rps->ref_dpb_index[0];
            av1Rps->ref_poc_array[4] = av1Rps->ref_poc_array[5] = av1Rps->ref_poc_array[6] = av1Rps->ref_poc_array[0];
        ...
        else if (picture_control_set_ptr->pred_struct_ptr->pred_type == EB_PRED_RANDOM_ACCESS)
        {
            ...
            printf("\t\tref_dpb_index (%d,%d,%d,%d,%d,%d,%d), ref_poc_array is (%d, %d, %d, %d, %d, %d, %d)\n",
                    ...
                    av1Rps->ref_dpb_index[2],
                    av1Rps->ref_dpb_index[3],
                    av1Rps->ref_dpb_index[4],
                    av1Rps->ref_dpb_index[5],
                    av1Rps->ref_dpb_index[6],
                    av1Rps->ref_poc_array[0],
                    av1Rps->ref_poc_array[1],
                    av1Rps->ref_poc_array[2],
                    av1Rps->ref_poc_array[3],
                    av1Rps->ref_poc_array[4],
                    av1Rps->ref_poc_array[5],
                    av1Rps->ref_poc_array[6]);
    ...
    {
        //Jing:TODO
        //Check for MRP case for trailing frames, disable the L0 which refers to trailing frames
        if (1)//MRP
        {
            uint8_t gop_i;
                ...
                case 4:
                    ...
                    if (picture_control_set_ptr->is_overlay) {
                        ...
                        if (pictureIndex == 0) {
                            ...
                            //{ -1, -3, -7, 0 }   // GOP Index 1 - Ref List 1
                            av1Rps->ref_dpb_index[LAST] = base1_idx; //2
                            av1Rps->ref_dpb_index[LAST2] = lay4_idx; //7
                            av1Rps->ref_dpb_index[LAST3] = lay1_0_idx; //4
                            av1Rps->ref_dpb_index[GOLD] = base0_idx; // 1
#endif

                            av1Rps->ref_dpb_index[BWD] = lay3_idx; //6
                            av1Rps->ref_dpb_index[ALT] = lay2_1_idx; //5
                            av1Rps->ref_dpb_index[ALT2] = lay1_1_idx; //3
                            gop_i = 1;
                            av1Rps->ref_poc_array[LAST] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, five_level_hierarchical_pred_struct[gop_i].ref_list0[0]);
                            av1Rps->ref_poc_array[LAST2] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, five_level_hierarchical_pred_struct[gop_i].ref_list0[1]);
                            av1Rps->ref_poc_array[LAST3] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, five_level_hierarchical_pred_struct[gop_i].ref_list0[2]);
    ...
    printf("\tOut Av1GenerateRpsInfo, hierarchical_levels is %d, slice_type %d, toggle is %d/%d/%d\n",
            picture_control_set_ptr->hierarchical_levels,
                    ...
                    (picture_control_set_ptr->pred_structure == EB_PRED_LOW_DELAY_B))
                {
...

                    {
                        if (sequence_control_set_ptr->static_config.hierarchical_levels == 1) {
                            ...
                            if (encode_context_ptr->pre_assignment_buffer_count > 1)
                            {
                                ...
                                else if (encode_context_ptr->pre_assignment_buffer_count == 8) {
                                    context_ptr->mini_gop_activity_array[L4_0_INDEX] = EB_FALSE;
                                    context_ptr->mini_gop_activity_array[L4_1_INDEX] = EB_FALSE;
                                } else if (encode_context_ptr->pre_assignment_buffer_count == 4) { 
                                    printf("---lalala, minigop 4 case---\n");
                                }

                                printf("----Dump mini_gop_activity_array-----\n");
                                for (int i = 0; i < 15; i++) {
                                    printf("mini_gop_activity_array idx %d, val %d\n", i, context_ptr->mini_gop_activity_array[i]);
                                }
                                printf("-------------------------------------\n");
                    ...
                    for (mini_gop_index = 0; mini_gop_index < context_ptr->total_number_of_mini_gops; ++mini_gop_index) {
                        ...
                        for (pictureIndex = context_ptr->mini_gop_start_index[mini_gop_index]; pictureIndex <= context_ptr->mini_gop_end_index[mini_gop_index]; ++pictureIndex) {
                            ...
                            printf("\n---[%u]: mini_gop_index %d, total_number_of_mini_gops %d, pictureIndex %d, mini_gop start/end %d/%d, pred_struct_period %d, hierarchical_layers_diff is %d, previous_mini_gop_length is %d\n",
                                    picture_control_set_ptr->picture_number,
                                ...
                                picture_control_set_ptr->cra_flag == EB_FALSE)
                            {
                                ...
                                printf("\tEnter trailing frames, pred_struct_position is %d, pred_struct_ptr set to %p\n",
                                        encode_context_ptr->pred_struct_position, picture_control_set_ptr->pred_struct_ptr);
                            }
                                        ...
                                        for (depIdx = 0; depIdx < depListCount; ++depIdx) {
                                            ...
                                            depPoc = POC_CIRCULAR_ADD(
                                                ...
                                                sequence_control_set_ptr->bits_for_picture_order_count*/);
                                                   ...
                                                   //Jing: shall we change it to IDR or CRA? currently will not ref if to CRA
                                                // If Dependent POC is greater or equal to the IDR POC
                            ...
                            if( (sequence_control_set_ptr->enable_altrefs == EB_TRUE &&
                                ...
                                else
                                {
                                    ...
                                    for (int pic_itr = 0; pic_itr <= num_past_pics; pic_itr++) {
                                        if (pictureIndex + pic_itr >= num_past_pics) {
                        ...
                        for (pictureIndex = context_ptr->mini_gop_start_index[mini_gop_index]; pictureIndex <= context_ptr->mini_gop_end_index[mini_gop_index] + has_overlay; ++pictureIndex) {
                            ...
                            if ((context_ptr->mini_gop_idr_count[mini_gop_index] == 0) &&
                                ...
                                printf("---set POC %d, decode order %d\n",
                                        picture_control_set_ptr->picture_number, picture_control_set_ptr->decode_order);
                            }
