    ...
    EbAsm                  asm_type)
{
    ...
    printf("--Generate Recon, plane %d, offset1d %d, tx type %d\n", plane, offset1d,  txb_ptr->transform_type[p_type]);
    ...
    while (blk_it < sequence_control_set_ptr->max_block_cnt) {
        ...
        if (part != PARTITION_SPLIT) {
            ...
            for (int32_t d1_itr = (int32_t)blk_it + offset_d1; d1_itr < (int32_t)blk_it + offset_d1 + num_d1_block; d1_itr++) {
                ...
                ///TODO: debug purpose
                //context_ptr->md_context->chroma_level = CHROMA_MODE_1;
                ///////////
                ...
                //assert(context_ptr->evaluate_cfl_ep == 0);
                ...
                if (cu_ptr->prediction_mode_flag == INTRA_MODE) {
                    ...
                    //Jing: one loop for CbCr due to CFL@Ep
                                ...
                                for (int i=0; i<=plane; i++) {
...
#ifdef DEBUG_REF_INFO1
                                    {
                                        ...
                                        int plane_originY = pu_block_origin_y;
                                        {
                                            ...
                                            dump_block_from_desc(txw, txh, reconBuffer, cu_originX, cu_originY, p);
                                        }
                                    }
#endif
                                    Av1EncodeGenerateRecon(
                    ...
                    if (cu_ptr->prediction_unit_array[0].merge_flag == EB_FALSE)
                    {
                        for (int32_t plane = 0; plane <= blk_geom->has_uv_ex; ++plane)
                        {
                            ...
                            for (tuIt = 0; tuIt < totTu; tuIt++) {
                                ...
                                //printf("kelvin ---> plane=%d, tuIt=%d, totTu=%d, plane_width=%d, plane_height=%d, txw=%d, twh=%d, sb_origin_xy=%d %d, txb_origin_xy=%d %d\n", plane, tuIt, totTu, plane_width, plane_height, txw, txh, sb_origin_x, sb_origin_y, txb_origin_x, txb_origin_y);
                                if (!zeroLumaCbfMD)
                                    //inter mode  1
                                    //Av1EncodeLoopFunctionTable[is16bit](
                                    Av1EncodeLoop(
