    ...
    for (;;) {
        ...
        if (pcs_ptr->parent_pcs_ptr->is_used_as_reference_flag == EB_TRUE &&
            ...
            for (uint16_t tile_idx = 0; tile_idx < tile_cnt; tile_idx++) {
                eb_av1_reset_cdf_symbol_counters(pcs_ptr->entropy_coding_info[tile_idx]->entropy_coder_ptr->fc);
                ((EbReferenceObject *)
                     pcs_ptr->parent_pcs_ptr->reference_picture_wrapper_ptr->object_ptr)
                    ->frame_context = (*pcs_ptr->entropy_coding_info[tile_idx]->entropy_coder_ptr->fc);
            }
#else
