...
static void reset_entropy_coding_picture(EntropyCodingContext *context_ptr,
    ...
    uint16_t tile_cnt = pcs_ptr->parent_pcs_ptr->av1_cm->tiles_info.tile_rows * pcs_ptr->parent_pcs_ptr->av1_cm->tiles_info.tile_cols;
    uint16_t tile_idx = 0;
    for (tile_idx = 0; tile_idx < tile_cnt; tile_idx++) {
        reset_bitstream(entropy_coder_get_bitstream_ptr(pcs_ptr->entropy_coding_info[tile_idx]->entropy_coder_ptr));
    }
    ...
    for (tile_idx = 0; tile_idx < tile_cnt; tile_idx++) {
        OutputBitstreamUnit *output_bitstream_ptr =
            (OutputBitstreamUnit *)(pcs_ptr->entropy_coding_info[tile_idx]->entropy_coder_ptr->ec_output_bitstream_ptr);
    //****************************************************************//
        uint8_t *data = output_bitstream_ptr->buffer_av1;
        pcs_ptr->entropy_coding_info[tile_idx]->entropy_coder_ptr->ec_writer.allow_update_cdf = !pcs_ptr->parent_pcs_ptr->large_scale_tile;
...
static EbBool update_entropy_coding_rows(PictureControlSet *pcs_ptr, uint32_t *row_index,
                                         uint32_t row_count, uint16_t tile_idx, EbBool *initial_process_call) {
...
void *entropy_coding_kernel(void *input_ptr) {
    ...
    for (;;) {
        ...
        const uint16_t tile_sb_start_x = cm->tiles_info.tile_col_start_mi[tile_col] >> scs_ptr->seq_header.sb_size_log2;
        const uint16_t tile_sb_start_y = cm->tiles_info.tile_row_start_mi[tile_row] >> scs_ptr->seq_header.sb_size_log2;
        uint16_t tile_width_in_sb =
            (cm->tiles_info.tile_col_start_mi[tile_col + 1] - cm->tiles_info.tile_col_start_mi[tile_col])
...

        {
            ...
            while (update_entropy_coding_rows(pcs_ptr,
                ...
                for (x_sb_index = 0; x_sb_index < tile_width_in_sb; ++x_sb_index) {
                    sb_index = (uint16_t)((x_sb_index + tile_sb_start_x) + (y_sb_index + tile_sb_start_y) * pic_width_in_sb);
                    ...
                    if (x_sb_index == 0 && y_sb_index == 0) eb_av1_reset_loop_restoration(pcs_ptr, tile_idx);

                    if (x_sb_index == 0 && y_sb_index == 0) {
                        context_ptr->tok = pcs_ptr->tile_tok[tile_row][tile_col];
                    }
                    sb_ptr->total_bits = 0;
                    uint32_t prev_pos = (x_sb_index == 0 && y_sb_index == 0) ? 0
                                                                            : pcs_ptr->entropy_coding_info[tile_idx]->entropy_coder_ptr->ec_writer.ec.offs;//residual_bc.pos
                    ...
                    write_sb(context_ptr,
                             ...
                             pcs_ptr->entropy_coding_info[tile_idx]->entropy_coder_ptr,
                             coeff_picture_ptr);
                    sb_ptr->total_bits = (pcs_ptr->entropy_coding_info[tile_idx]->entropy_coder_ptr->ec_writer.ec.offs - prev_pos)
                                         << 3;
                ...
                if (pcs_ptr->entropy_coding_info[tile_idx]->entropy_coding_tile_done == EB_FALSE) {
                    ...
                    if (pcs_ptr->entropy_coding_info[tile_idx]->entropy_coding_current_row == pcs_ptr->entropy_coding_info[tile_idx]->entropy_coding_row_count) {
                        uint32_t ref_idx;
