                ...
                if (input_entry_ptr->input_object_ptr != EB_NULL) {
                    ...
                    if (availability_flag == EB_TRUE) {
                        ...
                        uint32_t encDecSegColCnt =
                            entry_scs_ptr->enc_dec_segment_col_count_array[entry_pcs_ptr->temporal_layer_index];
                        uint32_t encDecSegRowCnt =
                            entry_scs_ptr->enc_dec_segment_row_count_array[entry_pcs_ptr->temporal_layer_index];
                        ...
                        TileInfo tile_info;
                        uint8_t tile_group_cols = MIN(tile_cols, entry_scs_ptr->tile_group_col_count_array[entry_pcs_ptr->temporal_layer_index]);
                        uint8_t tile_group_rows = MIN(tile_rows, entry_scs_ptr->tile_group_row_count_array[entry_pcs_ptr->temporal_layer_index]);
                        ...
                        // Jing:TODO: Updating tile group info
                        ...
                        for (uint8_t r = 0; r < tile_group_rows; r++) {
                            for (uint8_t c = 0; c < tile_group_cols; c++) {
                                ...
                                //uint16_t top_left_tile_idx = tile_group_col_start_tile_idx[c]
                                //    + tile_group_row_start_tile_idx[r] * tile_cols;
                                //uint16_t bottom_right_tile_idx = (tile_group_col_start_tile_idx[c + 1] - 1)
                                //    + (tile_group_row_start_tile_idx[r + 1] - 1) * tile_cols;
                                ...
                                TileGroupInfo *tg_info_ptr = &ppcs_ptr->tile_group_info[tile_group_idx];
                                ...
                                tg_info_ptr->tile_group_height_in_sb = tg_info_ptr->tile_group_sb_end_y - tg_info_ptr->tile_group_sb_start_y;
                                tg_info_ptr->tile_group_width_in_sb = tg_info_ptr->tile_group_sb_end_x - tg_info_ptr->tile_group_sb_start_x;
                                //printf("---TileGroupIdx %d,SB TL (%d, %d), BR (%d, %d), ColXRow %dX%d, Tile TL(%d,%d), BR(%d,%d)\n",
                                //    tile_group_idx, tg_info_ptr->tile_group_sb_start_x, tg_info_ptr->tile_group_sb_start_y,
                                //    tg_info_ptr->tile_group_sb_end_x, tg_info_ptr->tile_group_sb_end_y,
                                //    tg_info_ptr->tile_group_width_in_sb, tg_info_ptr->tile_group_height_in_sb,
                                //    tg_info_ptr->tile_group_tile_start_x,
                                //    tg_info_ptr->tile_group_tile_start_y,
                                //    tg_info_ptr->tile_group_tile_end_x,
                                //    tg_info_ptr->tile_group_tile_end_y);
                                ...
                                // Jing: Enable tile parallelism in Entropy Coding stage
                                // Entropy Coding Rows
                                for (uint16_t tr = top_left_tile_row_idx; tr < bottom_right_tile_row_idx; tr++) {
                                    for (uint16_t tc = top_left_tile_col_idx; tc < bottom_right_tile_col_idx; tc++) {
                                        uint16_t tileIdx = tr * tile_cols + tc;
                                        child_pcs_ptr->entropy_coding_info[tileIdx]->entropy_coding_current_row = 0;
                                        child_pcs_ptr->entropy_coding_info[tileIdx]->entropy_coding_current_available_row = 0;
                                        child_pcs_ptr->entropy_coding_info[tileIdx]->entropy_coding_row_count =
                                            (cm->tiles_info.tile_row_start_mi[tr + 1] - cm->tiles_info.tile_row_start_mi[tr]) >> sb_size_log2;
                                        child_pcs_ptr->entropy_coding_info[tileIdx]->entropy_coding_in_progress = EB_FALSE;
                                        child_pcs_ptr->entropy_coding_info[tileIdx]->entropy_coding_tile_done = EB_FALSE;

                                        for(unsigned rowIndex=0; rowIndex < MAX_SB_ROWS; ++rowIndex) {
                                            child_pcs_ptr->entropy_coding_info[tileIdx]->entropy_coding_row_array[rowIndex] = EB_FALSE;
                                        }
                                    }
                                }
                        ...
                        for (tile_row = 0; tile_row < tile_rows; tile_row++) {
                            ...
                            for (tile_col = 0; tile_col < tile_cols; tile_col++) {
                                ...
                                if (child_pcs_ptr->tile_tok[0][0]) {
                                    child_pcs_ptr->tile_tok[tile_row][tile_col] = pre_tok + tile_tok;
